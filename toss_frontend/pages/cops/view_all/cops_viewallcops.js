...
<DashboardLayoutHeader Title="View Cops" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <Image width="100" height="100"  className="m-auto w-10 h-10 rounded-full" src={"http:/localhost:3000/cops/getimage/"+item.ProfilePicture} alt={item.ProfilePicture}/>
                </td>
                <td className="px-6 py-4">
                    {item.Uname}
                </td>
...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/cops/all/manage_cops/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">View user</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  .then(response => {
    ...
    const response_viewallcops = await api.get('/cops/searchallcops/'+tossSession.inputValue)
