...
export default function ViewAllOfficerByCops({data,viewdata}) {
    ...
    viewallofficer_tablestyle.toss_table_full].join(' ')}>
        ...
        <tbody>
            ...
            viewallofficer_tablestyle.toss_table_data].join(' ')}>
                ...
                <Image width="100" height="100"  className="m-auto w-10 h-10 rounded-full" src={"http:/localhost:3000/officer/getimage/"+item.ProfilePicture} alt={item.ProfilePicture}/>                </td>
...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/cops/all/officers/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">View user</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallofficer = await api.get('/cops/searchallofficer/'+tossSession.inputValue)
