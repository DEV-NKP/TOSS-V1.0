...
import DashboardLayoutHeader from "@/pages/component/cops_component/cops_dashboard_layout_header";
import DashboardLayoutMenu from "@/pages/component/cops_component/cops_dashboard_layout_menu";
import Image from 'next/image'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'

import adminpagestyles from '../../../../../styles/cops_style/cops_viewprofile_style.module.css'
...
export default function ViewAllCasesByAdmin({data, casedata}) {
  ...
  const editCase = async (e)=>{
    ...
    router.push(
      ...
      '/cops/all/cases/modify/'+casedata.CaseId+'?Uname='+data.Uname
  );
  }
  const deleteCase = async (e)=>{
    e.preventDefault();
    const response = await api.delete('/cops/deletecasebyid/'+casedata.CaseId);
      router.push("/cops/manageCase/cops_viewallcase/?Uname="+data.Uname);
...
{/* ------------------   Header   --------------- */}
  ...
  <DashboardLayoutHeader  Title="Case Details" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
<div  className={[`w-full m-8 p-items-center overflow-auto relative z-0 mb-6 group`, 
       adminpagestyles.owner_profile_main].join(' ')}>

 <div className="grid grid-cols-1 pl-10 pr-10 pb-4 pt-8 sm:grid-cols-3">
  <div></div>
...
<div className="grid grid-cols-1 gap-3 pl-10 pr-10 pb-4 sm:grid-cols-2">
...
<div className="grid grid-cols-1 gap-3 pb-10 pl-4 sm:grid-cols-4">
  ...
  <button onClick={editCase} className={[` items-center justify-center pt-2.5 pb-2.5 pl-10 pr-10 mt-4  w-full max-w-xs font-medium`, 
       adminpagestyles.profile_bottom_button_area].join(' ')}  type="submit" >Edit Case<FontAwesomeIcon icon={faXmark} className={adminpagestyles.profile_icon}/> </button>
  </div>
  ...
  <button onClick={deleteCase} className={[` items-center justify-center pt-2.5 pb-2.5 pl-10 pr-10 mt-4  w-full max-w-xs font-medium`, 
       adminpagestyles.profile_bottom_button_area].join(' ')}  type="submit" >Delete Case <FontAwesomeIcon icon={faEraser} className={adminpagestyles.profile_icon}/> </button>
  </div>
  <div></div>
</div>
