...
export default function ViewAllSignUpByAdmin({data, signupdata}) {
...
{/* ------------------   Header   --------------- */}
  ...
  <AdminDashboardLayoutHeader  Title="Sign Up Details" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></AdminDashboardLayoutHeader>
...
<label className={[`relative z-0 w-full text-sm `, 
       adminpagestyles.owner_heading_label].join(' ')}>{signupdata.Uname}'s SignUp History</label>
<div></div>
</div>
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-2">
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
       adminpagestyles.owner_profile_lebel].join(' ')} > SignUp ID
    </p>
    ...
    <div className="pt-3">
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full`, 
       adminpagestyles.owner_profile_lebel].join(' ')} > User Email
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
       adminpagestyles.owner_profile_data].join(' ')} 
      >{signupdata.Email}
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
      adminpagestyles.owner_profile_lebel].join(' ')} > Time of SignUp
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
      adminpagestyles.owner_profile_data].join(' ')} 
      >{signupdata.Time}
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
      adminpagestyles.owner_profile_lebel].join(' ')} > User Post
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
      adminpagestyles.owner_profile_data].join(' ')} 
      >{signupdata.Post}
...
.then(response_admin => {
  ...
  var signupdata="";
  const SignUpId=context.query.SignUpId;
  const response = await api.get('/admin/viewsignupbysignupid/'+SignUpId);
  signupdata = await response.data;

return { props: { data,signupdata } }
