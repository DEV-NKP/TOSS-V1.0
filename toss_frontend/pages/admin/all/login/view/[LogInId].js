...
export default function ViewAllLogInByAdmin({data, logindata}) {
    ...
    const deleteLoginHistory = async (e)=>{
      e.preventDefault();
      const response = await api.delete('/admin/deleteloginbyid/'+logindata.LogInId);
...
{/* ------------------   Header   --------------- */}
  ...
  <AdminDashboardLayoutHeader  Title="Log In Details" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></AdminDashboardLayoutHeader>
...
<div className="grid grid-cols-3 justify-center gap-3 pb-10 pl-4">
  ...
  <div className="m-3 pt-0 ml-2">
  <button onClick={deleteLoginHistory} className={[` items-center justify-center pt-2.5 pb-2.5 pl-10 pr-10 mt-4  w-full max-w-xs font-medium`, 
       adminpagestyles.profile_bottom_button_area].join(' ')}  type="submit" >Delete LogIn Info <FontAwesomeIcon icon={faUnlockKeyhole} className={adminpagestyles.profile_icon}/> </button>
  </div>  
  </div>
  <div></div>
