...
export default function ViewAllReportsByAdmin({data, reportdata}) {

  const router= useRouter();

  const deletereport = async (e)=>{
    e.preventDefault();
    const response = await api.delete('/admin/deletereportbyid/'+reportdata.ReportId);
...
{/* ------------------   Header   --------------- */}
  ...
  <AdminDashboardLayoutHeader  Title="Report Details" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></AdminDashboardLayoutHeader>
...
<label className={[`relative z-0 w-full text-sm `, 
       adminpagestyles.owner_heading_label].join(' ')}>Report ID: {reportdata.ReportId}</label>
<div></div>
</div>
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-2">
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
       adminpagestyles.owner_profile_lebel].join(' ')} > Report ID
    </p>
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
      adminpagestyles.owner_profile_lebel].join(' ')} > Status
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
      adminpagestyles.owner_profile_data].join(' ')} 
      >{reportdata.Status}
...
<div className="grid grid-cols-3 gap-3 pb-10 pl-4">
  ...
  <div className="m-3 pt-0 ml-2">
  <button onClick={deletereport} className={[` items-center justify-center pt-2.5 pb-2.5 pl-10 pr-10 mt-4  w-full max-w-xs font-medium`, 
       adminpagestyles.profile_bottom_button_area].join(' ')}  type="submit" >Delete Report Info <FontAwesomeIcon icon={faUnlockKeyhole} className={adminpagestyles.profile_icon}/> </button>
  </div>  </div>
...
.then(response_admin => {
  ...
  var reportdata="";
  const ReportId=context.query.ReportId;
  const response = await api.get('/admin/viewreportbyid/'+ReportId);
  reportdata = await response.data;
