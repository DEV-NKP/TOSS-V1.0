...
import PageStyles_icon from '../../../styles/owner_style/owner_viewprofile_style.module.css'
import PageStyles from '../../../styles/shared_style/toss_button_style.module.css'
...
export default function ViewAllLogIn({data, viewdata}) {
  const downloadExcel = (e) => {
    e.preventDefault();
   const worksheet = XLSX.utils.json_to_sheet(viewdata[0].logins);
   const workbook = XLSX.utils.book_new();
   XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
   XLSX.writeFile(workbook, "LogIn_Info.xlsx");
 };
  return (
    <>
...
<DashboardLayoutHeader Title="My Log In Info" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
<div className={[`grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-1`, 
    viewalladmin_tablestyle.toss_table_full_search_div].join(' ')}>
       ...
       PageStyles.excel_download_button].join(' ')} onClick={downloadExcel}>
    Download Login Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,PageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            {viewdata[0].logins.map(item => (

            <tr key={item.LogInId} className={[`border-b dark:border-gray-700`, 
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                
                {/* <td scope="row" class="h-100 flex items-center px-6 py-4 text-gray-900">  
                  <Image width="150" height="150"  class="w-10 h-10 rounded-full" src={"http:/localhost:3000/owner/getimage/"+item.ProfilePicture} alt={item.ProfilePicture}/>
                    <div class="pl-3 pr-5">
                        <div class="text-base font-semibold">{item.Uname}</div>
                    </div>  
                </td> */}
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/login_history/view/"+item.LogInId+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View Details</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
var viewdata="";
const response_viewalladmin = await api.get('/admin/findloginbysignup/'+tossSession.Uname)
