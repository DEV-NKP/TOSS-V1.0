...
<DashboardLayoutHeader Title="My Log Out Info" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
<div className={[`grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-1`, 
    ...
    Download Logout Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,PageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/logout_history/view/"+item.LogOutId+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View Details</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
var viewdata="";
const response_viewalladmin = await api.get('/admin/findlogoutbysignup/'+tossSession.Uname)
