...
import {faAngleDown, faArrowRight, faCamera, faCircleUp, faCloudArrowUp, faHourglassStart, faPaperPlane, faReply, faShieldHalved, faSitemap, faTriangleExclamation, faUnlockKeyhole, faUpload, faUser, faUserPen, faUserTag} from '@fortawesome/free-solid-svg-icons'
...
export default function AdminProfile({data}) {
  ...
  const validateFile = (value) => {
      ...
      const handleFileUpload = async (event) => {
        ...
        try {
            const response = await api.put("/admin/updateprofilepicture/"+data.Uname,
                formData, {
                headers: {
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-3">
    ...
    <Image width="250" height="250" className={[`min-w-250 min-h-250` , 
       pagestyles.profile_img].join(' ')} src={"http:/localhost:3000/admin/getimage/"+data.ProfilePicture} alt={data.ProfilePicture} />
        <div className={pagestyles.border}>
      <div className={pagestyles.border_trans}></div>
    </div>
  </div>
<input onChangeCapture={handleFileUpload} 
accept=".jpg,.png,.gif,.jpeg,.PNG,.GIF,.JPG" 
className={[`block w-full m-4 text-sm rounded-lg cursor-pointer`, 
