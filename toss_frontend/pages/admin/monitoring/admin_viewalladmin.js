...
export default function ViewAdmin({data,viewdata}) {
  ...
  const downloadExcel = (e) => {
    ...
    XLSX.writeFile(workbook, "All_Admin.xlsx");
  };
...
<DashboardLayoutHeader Title="All Admin" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
{/* ------------------   Header   --------------- */}
    ...
    Download Admin Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
...
<tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/admin/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium  text-teal-300 hover:underline">View User</Link>
                </td>
            </tr>
            ))}
       </tbody>
...
.then(response => {
  ...
  const response_viewalladmin = await api.get('/admin/searchalladmin/'+tossSession.inputValue)
  ...
  // const response = await axios.get('http://localhost:3000/admin/viewalladmin');
