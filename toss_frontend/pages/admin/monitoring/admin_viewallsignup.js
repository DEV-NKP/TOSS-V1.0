...
export default function ViewAllSignUp({data, viewdata}) {
    ...
    const downloadExcel = (e) => {
      ...
      XLSX.writeFile(workbook, "All_SignUp.xlsx");
    };
...
<DashboardLayoutHeader Title="All SignUp" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
{/* ------------------   Header   --------------- */}
    ...
    viewalladmin_tablestyle.toss_table_full_search_div].join(' ')}>
        ...
        <div className={[`relative left-3`, 
         viewalladmin_tablestyle.toss_table_search].join(' ')}>
            ...
            dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for signup" 
             onChange={updateData}
             />
        </div>
    ...
    Download SignUp Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            <tr key={item.SignUpId} className={[`border-b dark:border-gray-700`, 
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/signup/view/"+item.SignUpId+"?Uname="+data.Uname} className="font-medium  text-teal-300 dark:text-blue-500 hover:underline">View Details</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
const response_viewalladmin = await api.get('/admin/searchallsignup/'+tossSession.inputValue)
