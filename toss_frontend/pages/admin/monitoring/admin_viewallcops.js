...
{/* ------------------   Header   --------------- */}
    ...
    Download COPS Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/cops/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View User</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
const response_viewalladmin = await api.get('/admin/searchallcops/'+tossSession.inputValue)
