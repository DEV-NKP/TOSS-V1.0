...
export default function ViewTransaction({data, viewdata}) {
  ...
  const updateData = async (e)=>{
    ...
    router.push({
      pathname: 'admin_viewalltransaction',
  ...
  const downloadExcel = (e) => {
    ...
    XLSX.writeFile(workbook, "All_Transaction.xlsx");
  };
...
{/* ------------------   Header   --------------- */}
    ...
    Download Transaction Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/transaction/view/"+item.TransactionId+"?Uname="+data.Uname} className="font-medium  text-teal-300 dark:text-blue-500 hover:underline">View transaction</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
const response_viewalladmin = await api.get('/admin/searchalltransaction/'+tossSession.inputValue)
