...
{/* ------------------   Header   --------------- */}
    ...
    Download Case Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/cases/view/"+item.CaseId+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View Case</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
.then(response => {
  ...
  const response_viewalladmin = await api.get('/admin/searchallcase/'+tossSession.inputValue)
