...
export default function ViewVli({data,viewdata}) {
  ...
  const downloadExcel = (e) => {
    ...
    XLSX.writeFile(workbook, "All_VLI.xlsx");
  };
...
{/* ------------------   Header   --------------- */}
    ...
    Download Vehicle License Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/vli/view/"+item.VliId+"?Uname="+data.Uname} className="font-medium  text-teal-300 dark:text-blue-500 hover:underline">View Vli</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
const response_viewalladmin = await api.get('/admin/searchallvli/'+tossSession.inputValue)
