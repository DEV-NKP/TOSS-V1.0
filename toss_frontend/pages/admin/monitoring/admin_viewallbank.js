...
export default function ViewBank({data, viewdata}) {
  ...
  const downloadExcel = (e) => {
    ...
    XLSX.writeFile(workbook, "All_Bank.xlsx");
  };
...
<DashboardLayoutHeader Title="All Account" userdata={data} Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
{/* ------------------   Header   --------------- */}
    ...
    viewalladmin_tablestyle.toss_table_full_search_div].join(' ')}>
        ...
        <div className={[`relative left-3`, 
         viewalladmin_tablestyle.toss_table_search].join(' ')}>
            ...
            dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for bank" 
             onChange={updateData}
             />
        </div>
    ...
    Download Bank Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,adminpagestyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewalladmin_tablestyle.toss_table_head].join(' ')}>
        ...
        <tbody>
            ...
            <tr key={item.AccountNo} className={[`border-b dark:border-gray-700`, 
            viewalladmin_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/admin/all/bank/view/"+item.AccountNo+"?Uname="+data.Uname} className="font-medium text-teal-300 hover:underline">View Account</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
.then(response => {
  ...
  const response_viewalladmin = await api.get('/admin/searchallbank/'+tossSession.inputValue)
