...
export default function OfficerEditProfile({data}) {
  ...
  const onSubmit = async (submitData) => {
      ...
      try {
          const response = await api.put("/officer/editprofile/"+data.Uname,
          { 
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-2">
  ...
  <label htmlFor="Designation" className={[` block mb-2`, 
       pagestyles.profile_lebel].join(' ')} >Designation</label>
  <input type="text" id="Designation" defaultValue={data.Designation} className={[` pb-1.5 pt-1.5 w-full`, 
       pagestyles.profile_data].join(' ')} 
