...
export default function OfficerProfile({data}) {
  ...
  router.push(
    {
      pathname: 'officer_changepassword',
      query: { Uname: data.Uname},
    }
  )
  }
  ...
  const validateFile = (value) => {
      ...
      const handleFileUpload = async (event) => {
        ...
        try {
            const response = await api.put("/officer/updateprofilepicture/"+data.Uname,
                formData, {
                headers: {
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-3">
    ...
    <div className="pt-3">
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full`, 
       pagestyles.admin_profile_lebel].join(' ')} > User Name
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
       pagestyles.admin_profile_data].join(' ')} 
      >{data.Uname}
    ...
    <div className="pt-3">
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full`, 
       pagestyles.admin_profile_lebel].join(' ')} > Designation
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
       pagestyles.admin_profile_data].join(' ')} 
      >{data.Designation}
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
      pagestyles.admin_profile_lebel].join(' ')} > Mobile No.
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
      pagestyles.admin_profile_data].join(' ')} 
      >{data.MobileNo}
...
<p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
  pagestyles.admin_profile_lebel].join(' ')} > Employee ID
</p>
<p className={[` pb-1.5 pt-1.5 w-full`, 
  pagestyles.admin_profile_data].join(' ')} 
  >{data.EmployeeId}
    ...
    <Image width="250" height="250" className={[`min-w-250 min-h-250` , 
       pagestyles.profile_img].join(' ')} src={"http:/localhost:3000/officer/getimage/"+data.ProfilePicture} alt={data.ProfilePicture} />
