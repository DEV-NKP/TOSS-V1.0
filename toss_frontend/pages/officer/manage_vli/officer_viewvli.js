...
export default function ViewVli({data, viewdata}) {
    ...
    const downloadExcel = (e) => {
      ...
      XLSX.writeFile(workbook, "All_Case.xlsx");
    };
...
<DashboardLayoutHeader Title="All Vli" userdata={data}  Uname={data.Uname} ProfilePicture={data.ProfilePicture}></DashboardLayoutHeader>
...
{/* ------------------   Header   --------------- */}
    ...
    viewallvli_tablestyle.toss_table_full_search_div].join(' ')}>
        ...
        <div className={[`relative left-3`, 
         viewallvli_tablestyle.toss_table_search].join(' ')}>
            ...
            dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for VLI" 
             onChange={updateData}
             />
        </div>
    ...
    Download Vehicle License Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,officerPageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewallvli_tablestyle.toss_table_full].join(' ')}>
        ...
        <tbody>
            ...
            viewallvli_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/officer/all/vli/view/"+item.VliId+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View VLI</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallvli = await api.get('/officer/searchallvli/'+tossSession.inputValue)
  .then(response_viewallvli => {
    // handle the response
    viewdata =  response_viewallvli.data;
