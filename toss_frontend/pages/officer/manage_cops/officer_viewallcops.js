...
export default function ViewAllCops({data, viewdata}) {
    ...
    const downloadExcel = (e) => {
      ...
      XLSX.writeFile(workbook, "All_COPS.xlsx");
    };
...
{/* ------------------   Header   --------------- */}
    ...
    Download COPS Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,officerPageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/officer/all/manage_cops/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View User</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallcops = await api.get('/officer/searchallcops/'+tossSession.inputValue)
