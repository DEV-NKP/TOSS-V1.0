...
export default function ViewAllOfficerByOfficer({data,viewdata}) {
    ...
    const updateData = async (e)=>{
      ...
      router.push({
        pathname: 'officer_viewallofficer',
...
{/* ------------------   Header   --------------- */}
    ...
    Download Officer Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,officerPageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/officer/all/officers/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View User</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallofficer = await api.get('/officer/searchallofficer/'+tossSession.inputValue)
