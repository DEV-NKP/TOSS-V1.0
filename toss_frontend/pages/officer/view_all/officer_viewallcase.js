...
{/* ------------------   Header   --------------- */}
    ...
    Download Case Info <FontAwesomeIcon icon={faDownload} className={[` pr-8`,officerPageStyles_icon.profile_icon].join(' ')}/>
</button>
</div>
    </div>
    ...
    viewallcases_tablestyle.toss_table_full].join(' ')}>
        ...
        <tbody>
            ...
            viewallcases_tablestyle.toss_table_data].join(' ')}>
                ...
                <td className="px-6 py-4">
                <Link href={"/officer/all/cases/view/"+item.CaseId+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View Case</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallcase = await api.get('/officer/searchallcase/'+tossSession.inputValue)
