...
export default function OwnerEditProfile({data}) {
  ...
  const onSubmit = async (submitData) => {
      ...
      try {
          const response = await api.put("/owner/editprofile/"+data.Uname,
          { 
...
<label htmlFor="DLN" className={[` block mb-2`, 
     pagestyles.profile_lebel].join(' ')} >Driving Licence No.</label>
<input type="text" id="DLN" defaultValue={data.DLN} className={[` pb-1.5 pt-1.5 w-full`, 
     pagestyles.profile_data].join(' ')} 
...
<div className="grid grid-cols-1 gap-3 pb-10 pl-4 sm:grid-cols-1">
