...
{item.Status === "ACTIVE"&& 
                ...
                <td className="px-6 py-4">
                <Link href={"/owner/all/owner/view/"+item.Uname+"?Uname="+data.Uname} className="font-medium text-teal-300 dark:text-blue-500 hover:underline">View User</Link>
                </td>
            </tr>
            ))}
        </tbody>
...
export async function getServerSideProps({query}) {
  ...
  const response_viewallowner = await api.get('/owner/searchallowner/'+tossSession.inputValue)
