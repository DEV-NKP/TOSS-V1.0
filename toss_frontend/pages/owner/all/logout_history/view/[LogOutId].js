...
<label className={[`relative z-0 w-full text-sm `, 
       ownerpagestyles.owner_heading_label].join(' ')}>{logoutdata.Uname}'s Log Out History</label>
<div></div>
</div>
...
<div className="grid grid-cols-1 gap-3 p-10 pr-4 sm:grid-cols-2">
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
       ownerpagestyles.owner_profile_lebel].join(' ')} > Log Out ID
    </p>
    ...
    <p  className={[` pb-2.5 pt-4 py-2.5 px-0 text-sm w-full `, 
      ownerpagestyles.owner_profile_lebel].join(' ')} > Time of LogOut
    </p>
    <p className={[` pb-1.5 pt-1.5 w-full`, 
      ownerpagestyles.owner_profile_data].join(' ')} 
      >{logoutdata.Time}
...
const response = await api.get('/owner/viewlogoutbylogoutid/'+LogOutId);
